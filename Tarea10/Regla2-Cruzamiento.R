...
for (rep in cruz){
  for (replica in 1:3){
    ...
    while(TRUE) {
      ...
      names(df) = c("Cruzamiento", "Replica", "Segundo", "Mejor", "Gap", "Optimo")
    }
  }
}
...
df %>%
  levene_test(Mejor~Cruzamiento)

#PRUEBA ESTADÍSTICA KRUSKAL WALLIS
kruskal.test(Mejor ~ Cruzamiento, data = df)

#PRUEBA WILCOXON
pairwise.wilcox.test(df$Mejor, df$Cruzamiento)
