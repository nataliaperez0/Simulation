...
ggplot(a, aes(x= Replica, y= Iteracion)) + 
  geom_line(color="black", size=1)+ #se puede cambiar a geom_line
  geom_point(shape=21, color="black", fill= "#F8766D", size=3)+
  theme_bw()+
  labs(x = "Replica", y = "Iteración con mayor filtración")

ggplot(b, aes(x= Replica, y= Iteracion)) + 
  geom_line(color="black", size=1)+ #se puede cambiar a geom_line
  geom_point(shape=21, color="black", fill= "#8800FF", size=3)+
  theme_bw()+
  labs(x = "Replica", y = "Iteración con mayor filtración")

ggplot(b, aes(x= Replica, y= Iteracion)) + 
  geom_line(color="black", size=1)+ #se puede cambiar a geom_line
  geom_point(shape=21, color="black", fill= "#FF8800", size=3)+
  theme_bw()+
  labs(x = "Replica", y = "Iteración con mayor filtración")
