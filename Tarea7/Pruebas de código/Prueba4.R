...
x <- seq(-2, 5, 0.25) #cambio de 0.4 a 0.5
y <-  x
z <- outer(x, y, g)
  ...
  for (repetir in 1:15){
    ...
    for (pot in 1:40) { #numero de imagenes a repeticiones dadas
      ...
      if (repetir == 1 & step == 0.25){
        ...
        lpoints(ux, uy, pch=20, col="red", cex=2)
        trellis.unfocus()
        trellis.focus("panel"[1], 1, 1, highlight=FALSE)
        lpoints(ux[maximo], uy[maximo], pch=20, col="blue",cex=3)
        trellis.unfocus()
    ...
    ultimo<-min(val)
    datos<- rbind(datos,c(step,repetir,ultimo))
  }
}
